...
window.dataAutocomplete = function (input, url) {
  $(input).autocomplete({
    source: url,
    minLength: 2,
...
window.getFile = function (element) {
  return $("#".concat(element)).prop('files')[0];
};

window.getChecked = function (element) {
  return $("#".concat(element, ":checked")).val();
};

window.getValueJquery = function (element) {
  return $("#".concat(element)).val();
};
